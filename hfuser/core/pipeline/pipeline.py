...
class hfuserPipeline(DiffusionPipeline): #基类
    def __init__(self):
        super().__init__()
    ...
    def __call__(self, *args, **kwargs): #重写__call__方法，支持diffusers的用法
        """
        In diffusers, it is a convention to call the pipeline object.
        But in hfuser, we will use the generate method for better prompt.
...
@dataclass
class hfuserPipelineOutput(BaseOutput):
    video: torch.Tensor